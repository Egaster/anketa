...
def html_for_court(answers):
    ...
    if '17' in answers and answers['17'] == 'yes':
        html+= """<p>Соответственно, если Вы понимаете, что не сможете оплатить долги по всем обязательствам в срок, это дает Вам возможность открыть процедуру судебного банкротства.</p>
        """
        if '19' not in answers or answers['19'] != 'no':
            html+="""<p><b>Неплатежеспособность</b> имеет место при соблюдении хотя бы одного из этих пунктов:</p>
