...
def form():
    ...
    if question_id < 0:
        ...
        return render_template('form.html', question=question, res=res, saved=saved, answers=session['answers'])
...
def html_for_court(answers):
    ...
    if '17' in answers and answers['17'] == 'yes':
        html+= """<p>Соответственно, если Вы понимаете, что не сможете оплатить долги по всем обязательствам в срок, а это дает Вам возможность открыть процедуру судебного банкротства.</p>
        <p>При этом, важно учитывать следующий момент: если у Вас нет доказательств того, что Вы не можете оплачивать задолженности вследствие непредвиденных обстоятельств (авария, болезнь, нахождение в больнице, инвалидность, появление нетрудоспособности и т.д.), то суд может не принять Ваше заявление или же отказать в признании Вас банкротом в ходе судебного процесса, так как сочтет Ваше поведение недобросовестным (например, когда лицо берет большое количество кредитов, зная заранее, что не будет в состоянии это выплатить). </p>
        <p><b>Неплатежеспособность</b> имеет место при соблюдении хотя бы одного из этих пунктов:</p>
    ...
    if '18' in answers and answers['18'] == 'no':
        html += "<p>При этом, важно учитывать следующий момент: если у Вас нет имущества, помимо, например, единственного жилья, то у Вас есть право подать в суд заявление о признании банкротом (<a href='https://www.consultant.ru/document/cons_doc_LAW_39331/c2c8c81ee8e4bd843286b08b10607f00ec6ae073/?ysclid=lumgijnp2z11735395' target='_blank'>ст. 213.4 ФЗ «О банкротстве»</a>).</p>"
    if '19' in answers and answers['19'] == 'no':
        html += """<p><b>Могут возникнуть трудности с принятием судом Вашего заявления о признании банкротом</b></p>
    ...
    if '19' in answers and answers['19'] == 'yes':
        html += """<p>Наличие уважительных причин невозможности выплатить в полном объеме все долги дает Вам право подать заявление в суд о признании Вас банкротом. 
        <br>К уважительным причинам и их подтверждению относятся, например, справки о временной нетрудоспособности, инвалидности, справка о ДТП и так далее.</p>""" 
